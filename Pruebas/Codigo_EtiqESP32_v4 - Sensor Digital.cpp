...
const int pin_FC2_DIGITAL = 27;  // Entrada digital del FC digital
const int pin_LED_FC2 = 26;      // LED indicador de estado FC2
bool FC2_detectada = false;
...
void setup() {
  ...
  pinMode(pin_FC2_DIGITAL, INPUT);  // Entrada digital fotodiodo
  pinMode(pin_LED_FC2, OUTPUT);     // LED de estado
...
void loop() {
  ...
  if (mover2 && etiquetapuesta && now > etiqueta_colocada + delay_etiqueta_contra) {
    ...
    FC2_detectada = digitalRead(pin_FC2_DIGITAL) == LOW;
  ...
  if (!mover1 && !mover2) {
    ...
    FC2_detectada = digitalRead(pin_FC2_DIGITAL) == LOW;
    digitalWrite(pin_LED_FC2, FC2_detectada ? HIGH : LOW);
    actualizar_tiempos();
    ...
    if (now - ultimo_refresco_lcd >= intervalo_lcd_idle) {
      ...
      lcd.print(FC2_detectada ? "YES " : "NO  ");
