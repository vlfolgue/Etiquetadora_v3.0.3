...
while (mover1 && actuador_fuera && (millis() > inicio_actuador + delay_botella_etiqueta) && llegada_botella != 0) {
    ...
    const int margen_histeresis = 10;  // Ajusta este valor tras observar valor_FC reales
    ...
    // Histéresis para estabilidad en la detección entre etiquetas
    static bool entreEtiquetas = false;
    if (!entreEtiquetas && valor_FC < valor_corte_FC_min - margen_histeresis) {
        entreEtiquetas = true;
    } else if (entreEtiquetas && valor_FC > valor_corte_FC_min + margen_histeresis) {
        entreEtiquetas = false;
    }

    if ((ahora > inicio_motor_etiqueta + 500) && !etiquetapuesta) {
        bool hayEtiqueta = valor_FC > valor_corte_FC_min;
        if ((FCentreetiquetas && !hayEtiqueta) || (!FCentreetiquetas && hayEtiqueta)) {
            etiquetapuesta = true;
        }
    }

    static bool led_estado = false;
    if (entreEtiquetas != led_estado) {
        digitalWrite(pin_estado_FC, entreEtiquetas ? HIGH : LOW);
        led_estado = entreEtiquetas;
    }

    if (entreEtiquetas && etiquetapuesta) {
        mover1 = false;
        digitalWrite(pin_Motor_Etiquetas, LOW);
    ...
    delayMicroseconds(100);  // Suaviza frecuencia de bucle sin perder velocidad
}

////// CONTRAETIQUETA
while (mover2 && etiquetapuesta && (millis() > etiqueta_colocada + delay_etiqueta_contra) && llegada_botella != 0) {
    unsigned long ahora = millis();
    const int margen_histeresis = 10;  // Ajusta este valor según tus lecturas reales
    ...
    // Aplicar histéresis para determinar si estamos entre etiquetas
    static bool entreContras = false;
    if (!entreContras && valor_FC2 < valor_corte_FC2_min - margen_histeresis) {
        entreContras = true;
    } else if (entreContras && valor_FC2 > valor_corte_FC2_min + margen_histeresis) {
        entreContras = false;
    }

    if ((ahora > inicio_motor_contra + 500) && !contrapuesta) {
        bool hayContra = valor_FC2 > valor_corte_FC2_min;
        if ((FCentrecontras && !hayContra) || (!FCentrecontras && hayContra)) {
            contrapuesta = true;
        }
    }

    static bool led_estado2 = false;
    if (entreContras != led_estado2) {
        digitalWrite(pin_estado_FC2, entreContras ? HIGH : LOW);
        led_estado2 = entreContras;
    }

    if (entreContras && contrapuesta) {
        mover2 = false;
        digitalWrite(pin_Motor_Contras, LOW);
    ...
    delayMicroseconds(100);  // Controla la frecuencia del bucle
}
